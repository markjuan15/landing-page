...
export function CardDirectory({ data, label }: Iprops) {

    const { dirtectoryModalState, toggleDirtectoryModalState } = useModalStates()
    const { themeState } = useTheme()

    function setData(datas: any) {
        useModalStates.setState({ directoryModalData: datas })
        toggleDirtectoryModalState(dirtectoryModalState)
    }
    ...
    return (
        <motion.div
            ...
            <span className="text-[1.2rem] text-center text-ellipsis line-clamp-4 uppercase">{formatLabel(label)}</span>
        </motion.div>
    )
}